...
public class WorkflowQListSubPanel extends TriviaPanel implements ActionListener {
	...
	public WorkflowQListSubPanel( TriviaInterface server, TriviaClient client ) {
		...
		while ( tryNumber < TriviaClient.MAX_RETRIES && success == false ) {
			tryNumber++;
			try {
				this.nQuestionsMax = server.getMaxQuestions();
				success = true;
			} catch ( RemoteException e ) {
				client.log( "Couldn't retrive number question from server (try #" + tryNumber + ")." );
			}
		}
	...
	public synchronized void update() {
		String[] openQuestionNumbers = null;
		String[] openQuestionText = null;
		String[] openQuestionValues = null;
		...
		while ( tryNumber < TriviaClient.MAX_RETRIES && success == false ) {
			tryNumber++;
			try {
				openQuestionNumbers = server.getOpenQuestionNumbers();
				openQuestionText = server.getOpenQuestionText();
				openQuestionValues = server.getOpenQuestionValues();
				success = true;
			} catch ( RemoteException e ) {
				client.log( "Couldn't retrive open question data from server (try #" + tryNumber + ")." );
			}
		}
