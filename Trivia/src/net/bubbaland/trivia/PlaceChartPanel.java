...
public class PlaceChartPanel extends TriviaPanel {
	 ...
	 */
	public PlaceChartPanel(TriviaInterface server, TriviaClient client) {
		...
		while ( tryNumber < TriviaClient.MAX_RETRIES && success == false ) {
			...
			try {
				for(int r=0; r<nRounds; r++) {
					newPlaces[r] = server.getAnnouncedPlace( r+1 );
					announced[r] = server.isAnnounced( r+1 );
					change = change || (newPlaces[r] != places[r]);
				}
				success = true;
			} catch ( Exception e ) {
				client.log( "Couldn't retrieve place data from server (try #" + tryNumber + ")." );
			}			
		}
