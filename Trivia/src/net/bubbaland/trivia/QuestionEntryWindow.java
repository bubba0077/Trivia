	...
	public QuestionEntryWindow(
		...
		if ( option == JOptionPane.OK_OPTION ) {
			...
			while ( tryNumber < TriviaClient.MAX_RETRIES && success == false ) {
				tryNumber++;
				try {
					int currentRound = server.getRoundNumber();
					if ( server.beenOpen( currentRound, qNumber ) ) {
						int existingQValue = server.getValue( currentRound, qNumber );
						String existingQText = server.getQuestionText( currentRound, qNumber );
