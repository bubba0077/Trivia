	...
	private class InternalScrollPanel extends TriviaPanel implements ActionListener {
		...
		public InternalScrollPanel(TriviaInterface server, TriviaClient	client) {
			...
			while ( tryNumber < TriviaClient.MAX_RETRIES && success == false ) {
				tryNumber++;
				try {
					this.nRounds = server.getNRounds();
					success = true;
				} catch ( Exception e ) {
			...
			while ( tryNumber < TriviaClient.MAX_RETRIES && success == false ) {
				...
				try {
					...
					for(int r=0; r<nRounds; r++) {
						...
						int earned = server.getEarned(r+1);
						int value = server.getValue(r+1);
						int announced = server.getAnnouncedPoints( r+1 );
						int place = server.getAnnouncedPlace( r+1 );
						String discrepency = server.getDiscrepencyText( r+1 );
						boolean isAnnounced = server.isAnnounced( r+1 );
					...
					client.log( "Couldn't retrieve round data from server (try #" + tryNumber + ")." );
				}
			...
			for(int r=0; r<nRounds; r++) {
				if(source.equals(this.discrepencyTextField[r])) {
					...
					while ( tryNumber < TriviaClient.MAX_RETRIES && success == false ) {
						tryNumber++;
						try {
							server.setDiscrepencyText(r+1, source.getText());
							success = true;
