...
public class WorkflowQueueSubPanel extends TriviaPanel implements ItemListener {
	...
	public WorkflowQueueSubPanel( TriviaInterface server, TriviaClient client ) {
		...
		for ( int a = 0; a < MAX_QUEUE_LENGTH; a++ ) {
			...
			constraints.gridx = 4;
			constraints.gridy = 2*a + 1;
			this.callerLabels[a] = enclosedLabel("", SUB_CALLER_WIDTH, ANSWER_HEIGHT/2, NOT_CALLED_IN_COLOR, BACKGROUND_COLOR, constraints, SMALL_FONT_SIZE, JLabel.CENTER, JLabel.CENTER);
			constraints.gridheight = 2;
	        
			constraints.gridx = 5;
			constraints.gridy = 2*a;
			this.operatorLabels[a] = enclosedLabel("", OPERATOR_WIDTH, ANSWER_HEIGHT, NOT_CALLED_IN_COLOR, BACKGROUND_COLOR, constraints, SMALL_FONT_SIZE, JLabel.CENTER, JLabel.CENTER);
	...
	public synchronized void update() {
		...
		String[] newTimestamps = trivia.getAnswerQueueTimestamps();
		int[] newQNumbers = trivia.getAnswerQueueQNumbers();
		String[] newAnswers = trivia.getAnswerQueueAnswers();
		int[] newConfidences = trivia.getAnswerQueueConfidences();
		String[] newSubmitters = trivia.getAnswerQueueSubmitters();
		String[] newOperators = trivia.getAnswerQueueOperators();
		String[] newCallers = trivia.getAnswerQueueCallers();
		String[] newStatuses = trivia.getAnswerQueueStatuses();
