...
public class CumulativePointsChartPanel extends TriviaPanel {
	 ...
	 */
	public CumulativePointsChartPanel( TriviaInterface server, TriviaClient client ) {
		...
		while ( tryNumber < TriviaClient.MAX_RETRIES && success == false ) {
			...
			try {
				for(int r=0; r<nRounds; r++) {
					newValues[r] = server.getValue( r+1 );
					newEarneds[r] = server.getEarned( r+1 );
					change = change || (newValues[r] != values[r]) || (newEarneds[r] != earneds[r]);
				}
				success = true;
			} catch ( Exception e ) {
				client.log( "Couldn't retrieve score data from server (try #" + tryNumber + ")." );
			}			
		}
