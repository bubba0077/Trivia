...
public class HeaderPanel extends TriviaPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent event) {
		...
		if ( source.equals( this.speedButton ) ) {
			...
			while ( tryNumber < TriviaClient.MAX_RETRIES && success == false ) {
				tryNumber++;
				try {
					lastRound = server.getRoundNumber() - 1;
					success = true;
				} catch ( RemoteException e ) {
	...
	public synchronized void update() {
		...
		int totalValue = -999;
		int currentRound = 0;
		int announcedPoints = -1;
		int announcedPlace = -1;
		...
		while ( tryNumber < TriviaClient.MAX_RETRIES && success == false ) {
			tryNumber++;
			try {
				roundEarned = server.getCurrentRoundEarned();
				roundValue = server.getCurrentRoundValue();
				totalEarned = server.getEarned();
				totalValue = server.getValue();
				speed = server.isCurrentSpeed();
				currentRound = server.getRoundNumber();
				roundOver = server.roundOver();
				isAnnounced = server.isAnnounced( currentRound - 1 );
				announcedPoints = server.getAnnouncedPoints( currentRound - 1 );
				announcedPlace = server.getAnnouncedPlace( currentRound - 1 );
				success = true;
			} catch ( RemoteException e ) {
				client.log( "Couldn't retrive header data from server (try #" + tryNumber + ")." );
			}
		}
		...
		this.roundEarnedLabel.setText( "" + roundEarned );
		this.totalEarnedLabel.setText( "" + totalEarned );
		this.roundValueLabel.setText( "" + roundValue );
		this.totalValueLabel.setText( "" + totalValue );
		this.currentHourLabel.setText( "Current Round: " + currentRound );
		if ( isAnnounced ) {
			this.announcedLabel.setText( "" + announcedPoints );
			this.placeLabel.setText( "" + announcedPlace );
		} else {
