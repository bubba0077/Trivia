...
public class NVisualTriviaDialog extends TriviaDialogPanel {
	...
	private final JSpinner		nVisualTrivia;
	 ...
	 */
	public NVisualTriviaDialog(TriviaClient client, int previousNVisualTrivia) {
		super();
		...
		c.weightx = 1.0;
		c.weighty = 1.0;
		this.nVisualTrivia = new JSpinner(new SpinnerNumberModel(client.getTrivia().getNVisual(), 1, 100, 1));
		this.nVisualTrivia.setFont(this.nVisualTrivia.getFont().deriveFont(fontSize));
		this.nVisualTrivia.addAncestorListener(this);
		this.add(this.nVisualTrivia, c);
		...
		if (option == JOptionPane.OK_OPTION) {
			...
			( new SwingWorker<Void, Void>() {
				...
				public Void doInBackground() {
					NVisualTriviaDialog.this.client.sendMessage(ClientMessageFactory
							.setNVisual(( (Integer) NVisualTriviaDialog.this.nVisualTrivia.getValue() ).intValue()));
					return null;
				}
